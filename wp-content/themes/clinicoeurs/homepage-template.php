...
<?php if (have_posts()): while (have_posts()):the_post(); ?>
	<main>
		...
		<Section class="services">
			...
			<?php foreach (get_field('service_section')['services'] as $services): ?>
				...
				<article class="tabs__content content">
					<img class="content__image" src="<?= $image_url ?>" alt="<?= __($alt_text, 'clinicoeurs') ?>">
		...
		<section class="support">
			...
			<img src="<?= $image_url ?>" alt="<?= __($alt_text, 'clinicoeurs') ?>">
		...
		<section class="volunteering">
			<h2 class="volunteering__title"> <?= __(get_field('volunteering_section')['section_title'], 'clinicoeurs') ?>  </h2>
			<p class="volunteering__text"> <?= __(get_field('volunteering_section')['section_text'], 'clinicoeurs') ?> </p>
			...
			<a href="<?= __(get_field('volunteering_section')['section_link'], 'clinicoeurs') ?>"
			   title="<?= __(get_field('volunteering_section')['section_link_title'], 'clinicoeurs') ?>">
				<?= __(get_field('volunteering_section')['section_link_label'], 'clinicoeurs') ?>
			</a>

			<p class="volunteering__insert">
				<?= __(get_field('volunteering_section')['volunteering_insert'], 'clinicoeurs') ?>
			</p>
		</section>
				...
				<a href="<?= __(get_field('category_link'), 'clinicoeurs') ?>"
				   title="<?= __(get_field('category_link_title'), 'clinicoeurs') ?>">
					<article class="product__card">
						<img src="<?= image(get_field('category_image'))['image_url'] ?>"
		...
		<section class="news news__section">
			...
			if ($recent_post->have_posts()): while ($recent_post->have_posts()):
				...
				<article class="job__card">
					...
					<img src="<?= image(get_field('article')['article_image'])['image_url'] ?>"
		...
		<section class="sponsor sponsor__section">
			<h3> <?= get_field('sponsor_section')['section_title'] ?> </h3>
			<?php foreach (get_field('sponsor_section')['sponsors_images'] as $sponsor): ?>
				<img src="<?= image($sponsor['sponsor_image'])['image_url'] ?>" alt="<?= __(image($sponsor['sponsor_image'])['alt_text'], 'clinicoeurs') ?>">
			<?php endforeach; ?>
		</section>
